...
class DeepSpeedEngine(Module):
    ...
    def forward(self, *inputs, **kwargs):
        ...
        #print(f"ds before {time.time() - time_s}")
        
        time_s = time.time()
        loss = self.module(*inputs, **kwargs)
        #print(f"ds loss {time.time() - time_s}")
        ...
        #print(f"ds after {time.time() - time_s}")
        if self.wall_clock_breakdown():
    ...
    def allreduce_bucket(self, bucket):
        ...
        self.allreduce_param_count += torch.numel(tensor)
