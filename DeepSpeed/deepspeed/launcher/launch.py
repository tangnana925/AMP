...
def main():
    ...
    # Check whether the rank_map is written
    ...
    if not os.path.exists(rmap_path):
        logger.info("inferring rank maps.")
        ...
        # load the pre-stored rank_map
        logger.info("loading rank maps.")
        with open(rmap_path) as json_file:
            rank_map = json.load(json_file)
        
        global_rank_mapping = dict()
        dist_world_size = 0
        for k, v in rank_map.items():
            node_name = node_list[int(k)]
            global_rank_mapping[node_name] = v
            dist_world_size += len(v)
