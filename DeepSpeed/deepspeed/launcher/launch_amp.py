...
def main():
    ...
    from deepspeed.amp import query_amp_topo, get_cluster
    cluster = get_cluster(world_info)
    global_rank_mapping, mp, pp = query_amp_topo(cluster)
   ...
   #      for gid in gids:
   #          global_rank_mapping[node_id].append(mapss[curr_global_rank])
   #          curr_global_rank += 1
    ...
    for local_rank in range(0, num_local_procs):
        ...
        cmd = [
            ...
            "--amp_model-parallel-size={}".format(mp),
            "--amp_pipe-parallel-size={}".format(pp)
